...
func _process(delta: float):
	...
	var is_inside = mouse_pos.x >= 0 and mouse_pos.x <= viewport_size.x and \
...
func _update_follow_positions(delta: float):
	...
	#     points[i] = lerp(points[i], selected_node_pos + (offset * (1.0 - weight)), weight * delta * 5.0)
